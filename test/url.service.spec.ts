...
describe('UrlService', () => {
  ...
  beforeEach(async () => {
    ...
    linkRepo = module.get(getRepositoryToken(Link)) as jest.Mocked<Repository<Link>>;
  });

  describe('create (code generation collision)', () => {
    it('should create link with unique code when no collision', async () => {
      linkRepo.findOne.mockResolvedValue(null);
      linkRepo.create.mockReturnValue({ ...mockLink, code: 'xyz98765' } as Link);
      linkRepo.save.mockImplementation((entity) => Promise.resolve({ ...entity, id: 'link-1' } as Link));
    ...
    it('should retry on code collision', async () => {
      ...
      linkRepo.findOne.mockImplementation(() => {
        callCount++;
        return Promise.resolve(callCount === 1 ? ({ code: 'taken' } as Link) : null);
      });
      linkRepo.create.mockReturnValue({ ...mockLink } as Link);
      linkRepo.save.mockImplementation((entity) => Promise.resolve(entity as Link));

      const result = await service.create('user-1', { originalUrl: 'https://x.com' });
  ...
  describe('getRedirectTarget', () => {
    ...
    it('should hit DB and cache on cache miss when link valid', async () => {
      ...
      expect(redis.setex).toHaveBeenCalledWith('short:abc12345', 3600, 'https://example.com');
    });
  ...
  describe('findOne', () => {
    ...
    it('should throw ForbiddenException when not owner and not admin', async () => {
      ...
      const otherUser = { ...mockUser, id: 'other-user', role: 'user' as const };
